...
class GcsBatchIoCommandSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  it should "test GcsBatchDeleteCommand" in {
    ...
    command.onSuccess(null, new HttpHeaders()).left.get
  ...
  it should "test GcsBatchSizeCommand" in {
    ...
    command.mapGoogleResponse(response) should be(139L)

    command.onSuccess(response, new HttpHeaders()).left.get should be(139L)
  ...
  it should "test GcsBatchCrc32Command" in {
    ...
    command.mapGoogleResponse(response) should be("aeiouy")

    command.onSuccess(response, new HttpHeaders()).left.get should be("aeiouy")
  ...
  it should "test GcsBatchTouchCommand" in {
    ...
    command.onSuccess(response, new HttpHeaders()).left.get
  ...
  it should "test GcsBatchExistsCommand" in {
    ...
    val response = new StorageObject()
    command.mapGoogleResponse(response) should be(true)

    command.onSuccess(response, new HttpHeaders()).left.get should be(true)
  ...
  it should "test GcsBatchIsDirectoryCommand" in {
    ...
    command.mapGoogleResponse(response) should be(false)
  ...
  it should "test GcsBatchCopyCommand" in {
    ...
    command.mapGoogleResponse(response) should be(())

    response.setDone(true)
    command.onSuccess(response, new HttpHeaders()).left.get should be(())

    response.setDone(false)
    response.setRewriteToken("token")
    command.onSuccess(response, new HttpHeaders()).right.get.rewriteToken should be(Option("token"))
  ...
  it should "test ExceptionSpewingGcsBatchIoCommand" in {
    ...
    the[RuntimeException] thrownBy {
      command.mapGoogleResponse(null)
    } should have message "Mapping that value is impossible"
    ...
    } should have message "exception in failure handler..."
  }
}
